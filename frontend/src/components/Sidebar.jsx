...
const Sidebar = ({ config, setConfig, models, prompts, selectedPrompt, onPromptSelect, defaults }) => {
    ...
    return (
        <aside className="w-full lg:w-80 xl:w-96 border-b lg:border-b-0 lg:border-r border-doc-border/40 flex flex-col bg-doc-sidebar/80 backdrop-blur z-20">
            <div className="px-6 py-5 border-b border-doc-border/30 shrink-0">
                <div className="flex items-center gap-4">
                    <img
                        src="/assets/logo.png"
                        alt="Docalypt logo"
                        className="w-12 h-12 rounded-2xl object-contain shadow-glow"
                    />
            ...
            <div className="flex-1 overflow-y-auto custom-scrollbar px-4 py-4 space-y-3">
                ...
                <CollapsibleSection
                    ...
                    isOpen={sections.model}
                    onToggle={() => toggleSection('model')}
                >
                    <div className="space-y-3 pt-2">
                        <div className="flex flex-col gap-1.5">
                            <label className="text-[9px] text-doc-text-dim font-semibold tracking-[0.2em] ml-1">PROVIDER</label>
                            <div className="relative group">
                                <select
                                    value={config.provider}
                                    onChange={(e) => handleChange('provider', e.target.value)}
                                    className="w-full text-xs font-medium bg-black/40 border border-white/5 focus:border-doc-accent/60 h-9 rounded-lg transition-all"
                                >
                        ...
                        <div className="flex flex-col gap-1.5">
                            <label className="text-[9px] text-doc-text-dim font-semibold tracking-[0.2em] ml-1">LOCAL MODEL</label>
                            <select
                                value={config.model}
                            ...
                            {config.provider !== 'ollama' && (
                                <div className="text-[9px] text-doc-text-dim mt-1 px-1">
                    ...
                    <div className="space-y-5 pt-2">
                        ...
                        <div className="flex flex-col gap-2 group">
                            <div className="flex items-center justify-between px-0.5">
                                ...
                                <span className="text-doc-accent font-mono text-[10px] font-semibold tracking-tight">{config.max_tokens}</span>
                            </div>
                            <input
                                type="number"
                                ...
                                className="w-full font-mono text-xs bg-black/40 border border-white/5 focus:border-doc-accent/60 rounded-lg"
                            />
                        </div>
                    ...
                    <div className="space-y-4 pt-2">
                        <div className="space-y-2">
                            <div className="flex justify-between items-center px-1">
                                ...
                                <button onClick={(e) => handleReset('template', e)} className="text-[10px] text-doc-accent hover:text-white transition-colors font-medium">Reset</button>
                            </div>
                            <div className="relative">
                                <button
                                    onClick={() => setShowPromptList(!showPromptList)}
                                    className="w-full text-left bg-black/40 border border-white/5 rounded-lg px-3 py-2.5 text-xs flex justify-between items-center hover:bg-black/50 transition-all group"
                                >
                                    <span className="truncate pr-2 font-medium text-white/90">{selectedPrompt?.title || 'User Template'}</span>
                                    <ChevronDown className={`w-3.5 h-3.5 text-doc-text-dim transition-transform duration-300 group-hover:text-white ${showPromptList ? 'rotate-180' : ''}`} />
                                </button>
                                {showPromptList && (
                                    <div className="absolute top-full left-0 right-0 mt-2 bg-[#161b22] border border-doc-border rounded-lg shadow-2xl z-50 max-h-60 overflow-y-auto custom-scrollbar ring-1 ring-black/50">
                                        {(prompts || []).map(p => {
                                            ...
                                            return (
                                                <div
                                                    ...
                                                    className={`p-3 cursor-pointer border-b border-white/5 last:border-0 transition-all flex items-start gap-3 ${isSelected
                                                        ? 'bg-doc-accent/10'
                                                        : 'hover:bg-white/5'
                                                        }`}
                                                >
                                                    <div className="mt-0.5">
                                                        {isSelected ? <div className="w-1.5 h-1.5 rounded-full bg-doc-accent shadow-[0_0_6px_rgba(56,200,155,0.8)]" /> : <div className="w-1.5 h-1.5" />}
                                                    </div>
                                                    <div className="flex-1 min-w-0">
                                                        <div className={`text-[11px] font-bold ${isSelected ? 'text-doc-accent' : 'text-white/90'}`}>
                                                            {p.title}
                                                        </div>
                                                        <div className="text-[10px] text-doc-text-dim truncate mt-0.5 font-medium opacity-70">
                                                            {p.description}
                                                        </div>
                                                    </div>
                                                </div>
                        ...
                        <div className="space-y-2">
                            <div className="flex justify-between items-center px-1">
                                ...
                                <button onClick={(e) => handleReset('persona', e)} className="text-[10px] text-doc-accent hover:text-white transition-colors font-medium">Reset</button>
                            </div>
                            <textarea
                                className="w-full min-h-[160px] bg-black/40 border border-white/5 rounded-lg p-3 text-xs leading-relaxed resize-none custom-scrollbar focus:border-doc-accent/60 outline-none transition-all placeholder:text-white/10 font-medium text-white/80"
            ...
            <div className="px-6 py-4 border-t border-doc-border/30 bg-black/20 shrink-0 backdrop-blur-sm">
                <div className="flex items-center justify-between px-1">
                    ...
                    <span className="text-[9px] text-doc-text-dim font-mono opacity-50">v1.1.0</span>
                </div>
            </div>
        </aside>
    );
...
const CollapsibleSection = ({ title, icon, children, isOpen, onToggle }) => (
    <div className="rounded-xl bg-black/20 border border-white/5 overflow-hidden transition-all duration-300">
        <button
            onClick={onToggle}
            className="w-full flex items-center justify-between p-4 hover:bg-white/5 transition-colors group"
        >
            <div className="flex items-center gap-2 text-[9px] font-bold text-doc-text-dim uppercase tracking-[0.2em] group-hover:text-doc-accent transition-colors">
                <span className="text-doc-accent/70 group-hover:text-doc-accent">{icon}</span>
                {title}
            </div>
            <ChevronDown className={`w-3.5 h-3.5 text-doc-text-dim transition-transform duration-300 ${isOpen ? 'rotate-180' : ''}`} />
        </button>
        <div
            className={`transition-[max-height,opacity] duration-300 ease-in-out px-4 overflow-hidden ${isOpen ? 'max-h-[800px] opacity-100 pb-4' : 'max-h-0 opacity-0'
                }`}
        >
            {children}
        </div>
